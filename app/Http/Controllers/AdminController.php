...
class AdminController extends Controller
{
    ...
    public function SendPasswordResetLink(Request $request){
        ...
        $oldToken = DB::table('password_reset_token')
        ->where(['email'=>$request->email,'guard'=>constGuards::ADMIN])
        ->first();
        if ($oldToken){
            //update token
            DB::table('password-reset-tokens')
                ->where(['email'=>$request->email,'guard'=>constGuards::ADMIN])
                ->update([
                    'token'=> $token,
            ...
            //add new token
            DB::table('password-reset-tokens')->insert([
                'email' => $request->email,
        ...
        if (sendEmail($emailConfig)){
            session()->flash('success','we have emailed reset password link');
    ...
    public function resetPassword(Request $request,$token = null){
        $check_token = DB::table('password_reset_tokens')
            ...
            ->first();
        if ($check_token) {
            $diffMins = Carbon::createFromDate('Y-m-d H-i-s', $check_token->create_at)
    ...
    public function  profileView(Request $request){
        ...
        return view('back.pages.admin.profile');
    }
    public function changeProfilePicture(Request $request){
        ...
        if ($upload){
            if ($old_picture != null && File::exists(public_path($path.$old_picture))){
                ...
                return response()->json(['status' => 1, 'msg' => 'yur profile picture updated']);
            }
    ...
    public function changefavicon(Request $request)
    {
        ...
        if ($upload){
            ...
            $update = $settings->save();
            return response()->json(['status' => 1, 'msg' => 'yur favicon has been updated']);
        }else{
            return response()->json(['status'=>0,'msg'=>'something went wrong']);
        }
    }
