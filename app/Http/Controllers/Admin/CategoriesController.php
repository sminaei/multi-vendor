...
class CategoriesController extends Controller
{
    ...
    public function storeSubCategory(Request $request)
    {
        ...
        if ($saved) {
            return redirect()->route('admin.manage-categories.add-subcategory')->with('success', '<b>' . ucfirst($request->subcategory_name) . '</b> sub category has been successfully added');
        } else {
    ...
    public function updateSubCategory(Request $request)
    {
        ...
        if ($subcategory->children->count() && $request->is_child_of != 0){
            return redirect()->route('admin.manage-categories.edit-subcategory',['id' => $subcategory_id])
                ->with('fail','this sub category has ('.$subcategory->children->count().')children. you can not change "is child of" option
                unless you free its a children');
        }else{
            ...
            $saved = $subcategory->save();
            if ($saved) {
                return redirect()->route('admin.manage-categories.edit-subcategory',['id'=>$subcategory_id])->with('success', '<b>' . ucfirst($request->subcategory_name) . '</b> sub category has been successfully added');
            } else {
